...
void init_digital_comm() {
	//Pin setup
	DDRB =  (DDRB & ~(HEAT_ON_OFF_PIN | PANCAKE_DONE_PIN)) | PANCAKE_DONE_PIN;	//Set Heat on/off as input and PANCAKE_DONE as output
	PORTB &= ~(PANCAKE_DONE_PIN);		//Set PANCAKE_DONE line to low and no pullup for heat on/off
	
	//Interrupt setup
	PCICR |= PCIE0;						//Enable pin change interrupt 1
